...
class AttValueMapperTests {
    ...
    fun `test mapper attribute value when data type is string`() {
        val result =
            CoreDynamoDbItemsMapper
                .getAttributeValue(TableTemplateItemMoshi
                    ("name", ItemType.STRING, false, "value", false, false))
        assert(result.s() == "value")
    }

    @Test
    fun `test mapper attribute value when data type is int`() {
        val result = CoreDynamoDbItemsMapper
            .getAttributeValue(
                TableTemplateItemMoshi("name", ItemType.INT, false,
                    "value", false, false))
    ...
    fun `test mapper attribute value when data type is boolean true`() {
        val result = CoreDynamoDbItemsMapper
            .getAttributeValue(
                TableTemplateItemMoshi("name", ItemType.BOOLEAN, false,
                    "true", false, false))
    ...
    fun `test mapper attribute value when data type is UTC date`() {
        val result = CoreDynamoDbItemsMapper
            .getAttributeValue(
                TableTemplateItemMoshi("name", ItemType.UTC_DATE, false,
                    "2025-04-20", false, false))
